...
export default function SuperAdminPage() {
  ...
  async function promoteEmployee() {
    ...
    try {
      ...
      await fetchApi(
        `/superadmin/employees/${promotion.employeeId}/permission-level`,
        {
          method: "PATCH",
          body: {
        ...
        <div className="flex flex-wrap items-start justify-between gap-5">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-6 text-gray-600">
              Review privileged activity, elevate trusted team members, and keep
              platform-level access changes inside one auditable workspace.
            </p>
          </div>
      ...
      {!loading && !isSuperAdmin ? (
        <div className="mt-6 rounded-card border border-amber-200 bg-amber-50 p-6 shadow-soft">
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-amber-900/80">
            This workspace is reserved for platform governance. Your account can
            still use the regular operations pages, but it cannot view audit
            logs or assign new admins.
          </p>
        </div>
      ) : null}
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(320px,0.88fr)]">
            <section className="rounded-card border border-border-soft bg-surface p-6 shadow-soft">
              ...
              <div className="mt-5 rounded-2xl border border-border-soft bg-surface-warm px-4 py-4 text-sm leading-6 text-gray-600">
                Promotions are written through the backend governance route and
                automatically logged to the platform activity stream.
              </div>
